...
# set breeding to 1 during summer and 0 in winter
breeding <- 1 - as.numeric((1:nWeeks) %% 52 %in% winterStart:winterEnd)

#set gamma to 0 during winter and 1 during summer
...
for(t in 2:nWeeks) {
  ...
  G[,t-1] <- rpois(nSites, gamma_seq[t-1]) # * N[,t-1])
  N[,t] <- S[,t-1] + G[,t-1] 
}
...
# change values in breeding to be index in jags
breeding[breeding==1]<-2
breeding[breeding==0]<-1
...
inits <- map(1:2, ~ list(gam = c(.8, .8), omega= .8, rho = .3, lam = 5))

Mod <- run.jags(model = "simulation_study/script/0_abMod_jags_seasonalgains.R",
                monitor = c("gam", "gamma", "omega", "rho", "p","lam", "n"),
                data = data.list,
                n.chains = 2,
                inits = inits,
                adapt = 5000,
...
save(mod, file="simulation_study/output/sim_output_seasonalgains_v2.RData")
